...
public partial class OrderService : IOrderService
{
        ...
        string orderNotes = null, int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false)
    {
        ...
        //wafaa - Account manager restriction (role 6): only show orders for their assigned customers
        //wafaa - Applied inside SearchOrdersAsync so no interface change is needed (binary compatible)
        try
        {
            ...
            if (currentCustomer != null)
            {
                ...
                var roleIds = await customerService.GetCustomerRoleIdsAsync(currentCustomer);
                bool isAccountManager = roleIds.Contains(6);
                bool isAdmin = roleIds.Contains(1);

                //wafaa - only apply when AM is browsing the admin (not filtering by specific customer)
                if (isAccountManager && !isAdmin && customerId == 0)
                {
                    var dataProvider = EngineContext.Current.Resolve<INopDataProvider>();
                    var amCustomerIds = await dataProvider.QueryAsync<int>(@"
                    ...
                    var idList = amCustomerIds.ToList();
                    if (!idList.Any()) idList.Add(-1);
                    query = query.Where(o => idList.Contains(o.CustomerId));
                }
            }
        }
        catch { }
