...
public partial class CustomerService : ICustomerService
{
    ...
    public CustomerService(CustomerSettings customerSettings,
        ...
        IRepository<CODFactors> codFactorsRepository = null,
        IRepository<CustomerType> customerTypeRepository = null)
    {
    ...
    public virtual async Task<IPagedList<Customer>> GetAllCustomersAsync(DateTime? createdFromUtc = null, DateTime? createdToUtc = null,
        ...
        int? erpCustomerId = null, int[]? accountManagerIds = null)
    {
        var customers = await _customerRepository.GetAllPagedAsync(async query =>
        {
            ...
            if (erpCustomerId.HasValue)
                query = query.Where(c => c.ERPCustomerId == erpCustomerId.Value);
            ...
            if (erpCustomerId.HasValue && erpCustomerId.Value > 0)
                query = query.Where(c => c.ERPCustomerId == erpCustomerId.Value);

            // Filter by account manager ids via AccountManager_CustomerMapping
            if (accountManagerIds != null && accountManagerIds.Length > 0)
            {
                var amIdsCsv = string.Join(",", accountManagerIds.Select(id => id.ToString()));
                var customerIdsForAM = await _dataProvider.QueryAsync<int>($@"
                    SELECT DISTINCT Customer_Id FROM AccountManager_CustomerMapping
                    WHERE AccountManagerId IN ({amIdsCsv})");
                var idList = customerIdsForAM.ToList();
                // If no customers mapped to selected AMs, force 0 results
                if (!idList.Any()) idList.Add(-1);
                query = query.Where(c => idList.Contains(c.Id));
            }

            //wafaa - Account manager restriction: role 6 = Account Manager, role 1 = Admin
            //wafaa - Account manager can only see customers assigned to them in AccountManager_CustomerMapping
            var currentCustomer = await EngineContext.Current.Resolve<IWorkContext>().GetCurrentCustomerAsync();
            var currentCustomerRoleIds = await GetCustomerRoleIdsAsync(currentCustomer);
            if (currentCustomerRoleIds.Contains(6) && !currentCustomerRoleIds.Contains(1))
            {
                //wafaa - Get Account_Manager.Id for the current logged-in account manager
                //wafaa - then get all customer IDs assigned to them from AccountManager_CustomerMapping
                var amCustomerIds = await _dataProvider.QueryAsync<int>(@"
                    ...
                    WHERE am.Customer_Id = @CurrentCustomerId
                      AND am.Active = 1
                      AND am.Deleted = 0",
                    new LinqToDB.Data.DataParameter("@CurrentCustomerId", currentCustomer.Id));

                var assignedIds = amCustomerIds.ToList();
                //wafaa - if no customers assigned, return empty result
                if (!assignedIds.Any()) assignedIds.Add(-1);
    ...
    public virtual async Task<IList<CODFactors>> GetAllCODFactorsAsync()
    {
        return await _codFactorsRepository.Table.ToListAsync();
    }

    public virtual async Task<IList<CustomerType>> GetAllCustomerTypeAsync()
    {
        return await _customerTypeRepository.Table.ToListAsync();
    }
