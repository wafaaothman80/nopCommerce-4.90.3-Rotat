...
public partial class ShipmentService : IShipmentService
{
    ...
    {
        //wafaa - Account manager restriction: get assigned customer IDs before query
        List<int> amCustomerIds = null;
        try
        {
            ...
            if (currentCustomer != null)
            {
                ...
                if (roleIds.Contains(6) && !roleIds.Contains(1) && orderId == 0)
                {
                    var dataProvider = EngineContext.Current.Resolve<INopDataProvider>();
                    var ids = await dataProvider.QueryAsync<int>(@"
                    ...
                    amCustomerIds = ids.ToList();
                    if (!amCustomerIds.Any()) amCustomerIds.Add(-1);
                }
            }
        }
        catch { }
        ...
        {
            //wafaa - apply account manager customer filter inside LINQ query (pagination-safe)
            if (amCustomerIds != null)
                query = from s in query
                        join o in _orderRepository.Table on s.OrderId equals o.Id
                        where amCustomerIds.Contains(o.CustomerId)
                        select s;
            if (orderId > 0)
