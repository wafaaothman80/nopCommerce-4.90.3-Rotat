...
public partial class ProductService : IProductService
{
    ...
    bool showHidden = false,
    bool? overridePublished = null,
    bool inStockOnly = false, bool discountOnly = false)
    {
        if (pageSize == int.MaxValue)
            pageSize = int.MaxValue - 1;
        ...
        if (!string.IsNullOrEmpty(keywords))
        {
            ...
            if (!runStandardSearch)
            {
                try
                {
                    providerResults = await activeSearchProvider.SearchProductsAsync(keywords, false);
                    productsByKeywords = providerResults.AsQueryable();
                }
                catch
                {
                    runStandardSearch = true;
                }
            }
        ...
        if (discountOnly)
        {
            // Apply the discount filter: Only products where OldPrice > Price
            productsQuery = productsQuery.Where(p => p.OldPrice > p.Price);
        }
        #region CATEGORY
        ...
        if (manufacturerIds != null && manufacturerIds.Count > 0)
        {
            ...
            productsQuery =
                from p in productsQuery
                join pm in productManufacturerQuery on p.Id equals pm
                select p;
        }
        ...
        if (filteredSpecOptions?.Count > 0)
        {
            foreach (var group in filteredSpecOptions.GroupBy(x => x.SpecificationAttributeId))
            {
                var optionIds = group.Select(x => x.Id).ToList();

                var productSpecQuery =
                    from psa in _productSpecificationAttributeRepository.Table
                    where psa.AllowFiltering && optionIds.Contains(psa.SpecificationAttributeOptionId)
                    select psa.ProductId;

                productsQuery =
                    from p in productsQuery
                    where productSpecQuery.Contains(p.Id)
                    select p;
            }
        }
        ...
        if (inStockOnly)
        {
            productsQuery = productsQuery.Where(p =>
                ...
                (
                    !p.UseMultipleWarehouses
                        ? p.StockQuantity > 0
                        : _productWarehouseInventoryRepository.Table
        ...
        var query = productsQuery.Select(p => new
        {
            Product = p,
            Stock =
                p.ManageInventoryMethodId != (int)ManageInventoryMethod.ManageStock
        ...
        return new PagedList<Product>(items, pageIndex, pageSize, totalCount);
    }
    protected virtual async Task<int> GetProductStockSortValueAsync(Product product)
    {
        if (product == null)
            return 0;

        // Treat non-managed stock as out of stock for sorting
        if (product.ManageInventoryMethodId != (int)ManageInventoryMethod.ManageStock)
            return 0;

        // Attribute combinations stock
        if (product.ManageInventoryMethodId == (int)ManageInventoryMethod.ManageStock &&
            product.ProductTypeId == (int)ProductType.SimpleProduct)
        {
            var combinationStock = await _productAttributeCombinationRepository.Table
                .Where(x => x.ProductId == product.Id)
                .SumAsync(x => (int?)x.StockQuantity);

            if (combinationStock.HasValue && combinationStock.Value > 0)
                return combinationStock.Value;
        }

        // Multiple warehouses
        if (product.UseMultipleWarehouses)
        {
            var warehouseStock = await _productWarehouseInventoryRepository.Table
                .Where(x => x.ProductId == product.Id)
                .SumAsync(x => (int?)x.StockQuantity);
