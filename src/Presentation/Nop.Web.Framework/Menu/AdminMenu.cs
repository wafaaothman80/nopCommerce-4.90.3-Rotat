...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    protected virtual async Task<AdminMenuItem> LoadMenuAsync(bool showHidden)
    {
        ...
        //wafaa - hide specific menu items for Account Manager role (role 6) only
        //wafaa - detect AM by: has CustomersView but NOT CustomerRolesView (which was removed for AMs)
        bool isAccountManagerOnly = customer.Id > 0
            && !await _permissionService.AuthorizeAsync(StandardPermission.Customers.CUSTOMER_ROLES_VIEW, customer)
            && await _permissionService.AuthorizeAsync(StandardPermission.Customers.CUSTOMERS_VIEW, customer);

        if (isAccountManagerOnly)
        {
            //wafaa - hide Online customers menu item for account managers
            var customersMenu = root.GetItemBySystemName("Customers");
            var onlineItem = customersMenu?.GetItemBySystemName("Online customers");
            if (onlineItem != null) onlineItem.Visible = false;
        }
