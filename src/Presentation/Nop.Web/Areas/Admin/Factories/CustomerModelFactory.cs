...
public partial class CustomerModelFactory : ICustomerModelFactory
{
    ...
    public virtual async Task<CustomerSearchModel> PrepareCustomerSearchModelAsync(CustomerSearchModel searchModel)
    {
        ...
        // Load active account managers for filter dropdown
        try
        {
            var ams = await _dataProvider.QueryAsync<Nop.Web.AccountManagerDto>(
            ...
            searchModel.AvailableAccountManagers = ams
                .Select(am => new SelectListItem { Value = am.Id.ToString(), Text = am.AccountManagerName })
                .ToList();
        }
        catch { }
    ...
    public virtual async Task<CustomerListModel> PrepareCustomerListModelAsync(CustomerSearchModel searchModel)
    {
        ...
        var accountManagerIds = searchModel.SelectedAccountManagerIds?.Any() == true
            ? searchModel.SelectedAccountManagerIds.ToArray()
            : null;
        ...
        var customers = await _customerService.GetAllCustomersAsync(customerRoleIds: searchModel.SelectedCustomerRoleIds.ToArray(),
            ...
            pageIndex: searchModel.Page - 1, pageSize: searchModel.PageSize, erpCustomerId: searchModel.SearchERPCustomerId,
        ...
        var model = await new CustomerListModel().PrepareToGridAsync(searchModel, customers, () =>
        {
            return customers.SelectAwait(async customer =>
            {
                ...
                //populate ERP fields from entity
                customerModel.ERPCustomerId = customer.ERPCustomerId ?? 0;
                customerModel.ERPRegisteredCIdsByPhone = customer.ERPRegisteredCIdsByPhone;
                customerModel.ERPRegisteredCIdsByEmail = customer.ERPRegisteredCIdsByEmail;
                customerModel.ERPIsRegisteredPhone = !string.IsNullOrEmpty(customer.ERPRegisteredCIdsByPhone);
                customerModel.ERPIsRegisteredEmail = !string.IsNullOrEmpty(customer.ERPRegisteredCIdsByEmail);
    ...
    public virtual async Task<CustomerModel> PrepareCustomerModelAsync(CustomerModel model, Customer customer, bool excludeProperties = false)
    {
        if (customer != null)
        {
            ...
            if (!excludeProperties)
            {
                ...
                //wafaa
                model.SelectedCODCountryId = customer.SelectedCODCountryId;
                model.SelectedCustomerTypeId = customer.SelectedCustomerTypeId;
                model.ERPCustomerIdToUpdate = customer.ERPCustomerIdToUpdate;
