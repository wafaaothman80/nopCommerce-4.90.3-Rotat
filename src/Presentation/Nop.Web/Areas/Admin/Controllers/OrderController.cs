...
public partial class OrderController : BaseAdminController
{
    ...
    //wafaa - Check if current account manager (role 6) has access to a specific customer's order
    //wafaa - Returns list of customer IDs assigned to current account manager, or null if admin
    protected virtual async Task<List<int>?> GetAccountManagerCustomerIdsAsync()
    {
        var currentCustomer = await _workContext.GetCurrentCustomerAsync();
        var roleIds = await _customerService.GetCustomerRoleIdsAsync(currentCustomer);

        //wafaa - Admin sees all orders
        if (roleIds.Contains(1)) return null;

        //wafaa - Account manager sees only their assigned customers
        if (roleIds.Contains(6))
        {
            MsSqlNopDataProvider db = new MsSqlNopDataProvider();
            var customerIds = await db.QueryAsync<int>(@"
                ...
                WHERE am.Customer_Id = @CurrentCustomerId
                  AND am.Active = 1 AND am.Deleted = 0",
    ...
    {
        //wafaa - Restrict order list to account manager's assigned customers only
        var amCustomerIds = await GetAccountManagerCustomerIdsAsync();
        if (amCustomerIds != null)
        {
            //wafaa - If no customers assigned, force empty result
            searchModel.AmCustomerIds = amCustomerIds.Any() ? amCustomerIds : new List<int> { -1 };
        }
    ...
    public virtual async Task<IActionResult> Edit(int id)
    {
        ...
        //wafaa - Account manager can only view orders for their assigned customers
        var amCustomerIds = await GetAccountManagerCustomerIdsAsync();
        if (amCustomerIds != null && !amCustomerIds.Contains(order.CustomerId))
            return AccessDeniedView();
