...
public partial class CatalogController : BasePublicController
{
    ...
    public virtual async Task<IActionResult> Category(int categoryId, CatalogProductsCommand command)
    {
        ...
        //by wafaa 25-6
        command.InStockOnly = Request.Query.ContainsKey("instockonly") &&
                      bool.TryParse(Request.Query["instockonly"], out var InStockOnly) &&
                      InStockOnly;
        command.DiscountOnly = Request.Query.ContainsKey("discountonly") &&
                          bool.TryParse(Request.Query["discountonly"], out var discountOnly) &&
                          discountOnly;
        //model
