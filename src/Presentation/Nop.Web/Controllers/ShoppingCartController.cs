    ...
    public virtual async Task<IActionResult> AddItemsToCartFromWishlist(Guid? customerGuid, WishlistModel wishlistModel, IFormCollection form)
    {
        ...
        foreach (var sci in pageCart)
        {
            if (allIdsToAdd.Contains(sci.Id))
            {
                ...
                var quantity = form.ContainsKey(formKey) && int.TryParse(form[formKey], out var parsedQty) && parsedQty > 0
                    ? parsedQty
                    : sci.Quantity;
                ...
                var warnings = await _shoppingCartService.AddToCartAsync(customer,
                    ...
                    sci.RentalStartDateUtc, sci.RentalEndDateUtc, quantity, true);
                if (!warnings.Any())
                    countOfAddedItems++;
