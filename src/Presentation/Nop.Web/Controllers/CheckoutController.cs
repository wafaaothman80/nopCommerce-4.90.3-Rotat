...
using static Nop.Web.Controllers.HomeController;
...
public partial class CheckoutController : BasePublicController
{
    ...
    public CheckoutController(HttpClient client, AddressSettings addressSettings,
    ...
    public virtual async Task<IActionResult> Index()
    {
        ...
        //by wafaa 26-6 // update customer wallet with erp data
        ...
        _logger.Information($"[CreditAndInvoices DEBUG] URL={url}");
        _logger.Information($"[CreditAndInvoices DEBUG] StatusCode={response.StatusCode} | Content={response.Content}");
    ...
    protected virtual async Task CreateErpOrderAndChargeCreditAsync(Order order, Customer customer)
    {
        ...
        if (existingErpOrderId.HasValue && existingErpOrderId.Value > 0)
        {
            ...
            MsSqlNopDataProvider msSqlNopDataProvider0 = new MsSqlNopDataProvider();
            DataParameter[] parameters0 = { new DataParameter(name: "@CID", value: customer.Id) };

            var ERPCustomer = msSqlNopDataProvider0.QueryProcAsync<CustomerModel>("GetCustomerERPRegistered", parameters0);
            ...
            var payload = new
            {
                objType = 11,
                objId = "",
                sales_order_print_name = "1",
                order_type = "3",
                customer = ERPCustomer.Result.FirstOrDefault().ERPCustomerId.ToString(),
                primary_contact = "",
                currency = "1",
                exchangerate = "1.0",
                location = "1",
                ...
                emirate = "",
                next_role_approval = "",
                lines = orderItems.Select(line => new
                {
                    item = line.ProductId.ToString(),
                    location = "1",
                    branch = "1",
                    department = "11",
                    quantity = line.Quantity.ToString(),
                    rate = decimal.Parse(
                        line.OriginalProductCost.ToString(CultureInfo.InvariantCulture),
                        CultureInfo.InvariantCulture
                    ),
                    inventory_details = Array.Empty<object>()
                }).ToList()
            };
            ...
            await _logger.InformationAsync($"ERP orderItems.Count = {orderItems?.Count ?? 0}");

            await _logger.InformationAsync("ERP payload => " + payload);
            ...
            if (httpResponse.StatusCode == HttpStatusCode.OK)
            {
                ...
                var userData = JsonConvert.DeserializeObject<OredrResponceModel>(responseJson);

                var ERPOrderId = userData.ns_id;

                var msSqlNopDataProvider1 = new MsSqlNopDataProvider();
                DataParameter[] parameters1 =
                {
        new DataParameter("@ERPOrderId", ERPOrderId),
        new DataParameter("@OrderId", order.Id)
    };

                var ERPOrderId0 = await msSqlNopDataProvider1.QueryProcAsync<int>("OrderSaveERPId", parameters1);

                await _logger.InformationAsync($"ERPOrder ({ERPOrderId0}) completed. ns_id: {userData.ns_id}, message: {userData.message}");
                // create sda order
