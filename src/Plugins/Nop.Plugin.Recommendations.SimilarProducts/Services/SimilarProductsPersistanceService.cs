...
namespace Nop.Plugin.Recommendations.SimilarProducts.Services
{
    public class SimilarProductsPersistanceService : ISimilarProductsPersistanceService
    {
        ...
        // جديد: إدراج يدوي لسطر واحد
        public async Task InsertAsync(SimilarProductRecord record)
        {
            if (record == null)
        ...
        {
            var entity = await _similarProductRecordRepository.GetByIdAsync(id);
            if (entity != null)
                await _similarProductRecordRepository.DeleteAsync(entity);
        }

        // جديد: بحث بسيط + ترتيب بالـ Similarity desc
        public async Task<IPagedList<SimilarProductRecord>> SearchAsync(
            int? productId = null,
            int? similarProductId = null,
            int pageIndex = 0,
            int pageSize = 20)
        {
            return await _similarProductRecordRepository.GetAllPagedAsync(query =>
            {
                if (productId.HasValue)
                    query = query.Where(x => x.ProductId == productId.Value);
                if (similarProductId.HasValue)
                    query = query.Where(x => x.SimilarProductId == similarProductId.Value);

                return query.OrderByDescending(x => x.Similarity)
