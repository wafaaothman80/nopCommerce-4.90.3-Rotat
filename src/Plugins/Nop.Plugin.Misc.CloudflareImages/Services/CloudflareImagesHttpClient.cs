...
public class CloudflareImagesHttpClient
{
    ...
    public CloudflareImagesHttpClient(CloudflareImagesSettings cloudflareImagesSettings, HttpClient httpClient)
    {
        ...
        // Use a longer timeout for image uploads (minimum 120s regardless of setting)
        var configuredSeconds = _cloudflareImagesSettings.RequestTimeout ?? CloudflareImagesDefaults.RequestTimeout;
        httpClient.Timeout = TimeSpan.FromSeconds(Math.Max(configuredSeconds, 120));
    ...
    /// <summary>
    /// Applies requireSignedURLs + allowed_variant metadata to an existing image.
    /// NOTE: Cloudflare re-creates the image under a NEW id; the returned Result.Id
    /// is that new id and MUST replace the stored ImageId.
    /// </summary>
    /// <param name="imageId">Current Cloudflare image identifier</param>
    /// <param name="allowedVariant">Metadata value, e.g. "public"</param>
    public async Task<CloudflareImagesResponse> SetSignedUrlAsync(string imageId, string allowedVariant = "public")
    {
        var json = JsonConvert.SerializeObject(new
        {
            requireSignedURLs = true,
            metadata = new Dictionary<string, string> { ["allowed_variant"] = allowedVariant }
        });

        var content = new StringContent(json, System.Text.Encoding.UTF8, "application/json");
        var result = await RequestAsync($"/{imageId}", content, HttpMethod.Patch);
        return JsonConvert.DeserializeObject<CloudflareImagesResponse>(result);
    }
